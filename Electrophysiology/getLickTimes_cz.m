...
%%
function behaviorEvents = getLickTimes(npEvents,behaviorEvents,fig)
...
for i = 1:nTrials
    ...
    if isempty(firstLeft) && ~isempty(firstRight)
        ...
        behaviorEvents(i).FirstLatBpod=behaviorEvents(i).RightLicks(1);
    elseif isempty(firstRight) && ~isempty(firstLeft)
        behaviorEvents(i).firstLateral = 1;
        behaviorEvents(i).FirstLatBpod=behaviorEvents(i).LeftLicks(1);
    elseif ~isempty(firstRight) && ~isempty(firstLeft)
        [~,behaviorEvents(i).firstLateral] = min([firstLeft firstRight]);
        behaviorEvents(i).FirstLatBpod=NaN;
    end
end
...
for i = 1:nTrials
    ...
    if ~isempty(behaviorEvents(i).RightLicksNP)
        behaviorEvents(i).FirstLatNP = behaviorEvents(i).RightLicks(1);
    elseif ~isempty(behaviorEvents(i).LeftLicks)
        behaviorEvents(i).FirstLatNP= behaviorEvents(i).LeftLicks(1);      
    end
end


fprintf('Removed %d lateral licks and %d central licks from NP data\n',[aCount + bCount,cCount]);
