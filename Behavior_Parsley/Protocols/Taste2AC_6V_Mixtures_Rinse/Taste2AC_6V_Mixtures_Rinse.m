function Taste2AC_6V_Mixtures_Rinse   
global BpodSystem
...
if S.GUI.TrainingLevel ~=4
    ...
    TrialTypes = repmat(trialseq,1,60);
else
...
for currentTrial = 1:MaxTrials
    ...
    if S.GUI.TrainingLevel ~=5 % context
        switch TrialTypes(currentTrial)
            ...
            case 1 
                 if ismember(ValveSeq(currentTrial),[1:3])
          ...
          case 2 % right trials; delivery of tastant from line 2
                 if ismember(ValveSeq(currentTrial),[4:6])
                     valveID = 2*ValveSeq(currentTrial)-1;
                 end
  ...
  if S.GUI.TrainingLevel ~=2  % all other case, meaning not correction trials; (include the habituation + no_correction)
    ...
    sma = AddState(sma, 'Name', 'AspirationDown', ...
        ...
        'OutputActions', {'SoftCode', 6});
         %start of rinse
    sma = AddState(sma, 'Name', 'RinseValveOn', ... %Open valve #7 to deliver 4uL water
        'Timer', valvetimes(7)*0.6,...
        'StateChangeConditions ', {'Tup', 'RinseValveOff'},...
        'OutputActions', {'ValveModule1', 13,'BNCState',1}); 
    
     sma = AddState(sma, 'Name', 'RinseValveOff', ... % close valve #7
        'Timer', 0.01,...
        'StateChangeConditions', {'Tup', 'VacOn'},...
        'OutputActions', {'ValveModule1', 14,'BNCState',0});
