close all;
SoundFreqLeft = 4000;
SoundFreqRight = 13000;
SF = 52000;
SoundDuration=1;
Left = GenerateSineWave(SF, SoundFreqLeft, SoundDuration);
Right = GenerateCosWave(SF, SoundFreqRight, SoundDuration);
Right2 = GenerateSineWave(SF, SoundFreqRight, SoundDuration);


figure(15);
plot(Right2(1:100)); hold on;
plot(Right(1:100));

function CosWave = GenerateCosWave(SamplingRate, Frequency, Duration)
% Duration in seconds
dt = 1/SamplingRate;
t = 0:dt:Duration;
CosWave=cos(2*pi*Frequency*t);
end
...
if isempty(fieldnames(S))  % If chosen settings file was an empty struct, populate struct with default settings
  ...
  %     S.GUI.SamplingDuration = 5;
    S.GUI.SoundDuration = 2; % Duration of sound (s)
    ...
    S.GUI.DelayDuration = 0.5;
    S.GUI.TastantAmount = 0.05;
    S.GUI.MotorTime = 0.5;
    S.GUI.Up        = 14;
    S.GUI.Down      =   5;
    S.GUI.ResponseTime = 0; %3
    S.GUI.DrinkTime = 0;
    S.GUI.RewardAmount = 5; % in ul
    S.GUI.PunishTimeoutDuration = 0; %10
...
OldLeft = GenerateSineWave(SF, S.GUI.SoundFreqLeft, S.GUI.SoundDuration);
NewLeft = (GenerateSineWave(SF, S.GUI.SoundFreqLeft, S.GUI.SoundDuration))*0.8;
OldRight = GenerateSineWave(SF, S.GUI.SoundFreqRight, S.GUI.SoundDuration);
NewRight = GenerateSineWave(SF, S.GUI.SoundFreqRight, S.GUI.SoundDuration);
NewRight(NewRight>0.68)=0.68;
NewRight(NewRight<-0.68)=-0.68;

W.loadWaveform(1, OldLeft);
W.loadWaveform(2, NewLeft);
W.loadWaveform(3, OldRight);
W.loadWaveform(4, NewRight);
...
figure(15);
plot(1:100, OldLeft(1:100)); hold on;
plot(1:100, NewLeft(1:100)); hold on;
plot(1:100, OldRight(1:100)); hold on;
plot(1:100, NewRight(1:100)); hold off;
legend(['old left'],['new left'],['old right'], ['new right'])
...
%% Main loop (runs once per trial)
 ...
 left_count = 0;%zeros(1,BpodSystem.Data.nTrials);
 left_correct = 0;%zeros(1,BpodSystem.Data.nTrials);
 right_count = 0;%zeros(1,BpodSystem.Data.nTrials);
 right_correct = 0; %zeros(1,BpodSystem.Data.nTrials);
    ...
    if S.GUI.TrainingLevel ~=5 % context
        switch TrialTypes(currentTrial)
            ...
            case 2 % right trials; delivery of tastant from line 2
                ...
                centralvalvetime = valvetimes((valveID+1)/2);
           case 3 % right trials; delivery of tastant from line 2
                valveID = v2;
                soundValue = 3; % 13000
                leftAction = 'Timeout'; rightAction = 'reward';
                ValveCode = 2; ValveTime = RightValveTime; % reward, valve2 = right spout
...
%         'OutputActions', {'SoftCode', 1});
    ...
    sma = AddState(sma, 'Name', 'SoundOn', ... % Sound is on for 2 s and
        ...
        'OutputActions', {'WavePlayer1', soundValue}); 
